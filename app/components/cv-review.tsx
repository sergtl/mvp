...
function CVReview({
  ...
  // updates data on top of what was parsed
  const mutation = useMutation({
    ...
    onSuccess: async (_, values) => {
      ...
      // invalidate cache when user updates data
      await onSaved();
    },
  });
    ...
    >
      ...
      <fieldset disabled={mutation.isPending} className="space-y-4">
        ...
        {(["name", "email", "phone", "location"] as const).map((name) => (
          <label key={name} className="block space-y-1 text-sm capitalize">
            {name}
            <Input {...form.register(`contact.${name}`)} />
          </label>
        ))}
        ...
        <label className="block text-sm">
          Summary
          <Textarea className={areaClass} {...form.register("summary")} />
        </label>

        <h3 className="font-semibold">Experience</h3>
        {experience.fields.map((entry, i) => (
          <div key={entry.id} className="space-y-2 rounded border p-3">
            {(["company", "title", "startDate", "endDate"] as const).map(
              (name) => (
                <label key={name} className="block text-sm">
                  ...
                  <Input {...form.register(`experience.${i}.${name}`)} />
                </label>
              ),
            )}
            <label className="block text-sm">
        ...
        {education.fields.map((entry, i) => (
          <div key={entry.id} className="space-y-2 rounded border p-3">
            {(
              ["institution", "qualification", "startDate", "endDate"] as const
            ).map((name) => (
              <label key={name} className="block text-sm">
                {
                  {
                ...
                <Input {...form.register(`education.${i}.${name}`)} />
              </label>
            ))}
            <Button
              type="button"
        ...
        <Button type="submit" disabled={mutation.isPending}>
          {mutation.isPending ? "Saving…" : "Save reviewed CV"}
        </Button>
      </fieldset>
