...
export async function GET(request: Request, context: Context) {
  ...
  // getting latest parse job
  const [parse] = await db
    .select()
    .from(cvParse)
    .where(eq(cvParse.cvId, id))
  ...
  const [review] = await db
    .select()
    .from(cvReview)
    .where(eq(cvReview.cvId, id));

  return Response.json(
    { parse: parse ?? null, review: review ?? null },
...
export async function POST(request: Request, context: Context) {
  ...
  if (access.record.contentType !== "application/pdf")
    return Response.json(
      { error: "Only PDFs can be parsed." },
      { status: 415 },
    );
