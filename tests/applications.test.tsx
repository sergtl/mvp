...
import { SavedApplicationForm } from "../app/app/applications/saved-form";
...
async function main() {
  ...
  const html = renderToStaticMarkup(
    <SavedApplicationForm
      id="saved"
      snapshot={snapshot}
      files={[{ id: "file", fieldId: "0-2-0", filename: "original.pdf" }]}
    />,
  );
  ...
  for (const field of html.matchAll(/<(?:input|textarea|select)\b[^>]*>/g))
    assert.match(field[0], /disabled=""/);
  ...
  try {
    ...
    const { user, submission, submissionFile } =
      await import("../lib/db/schema");
    const { listApplications, getApplication, getApplicationFile } =
      await import("../lib/applications/data");
    ...
    const [record] = await db
      ...
      .values({
        userId: owner,
        sourceURL: snapshot.job.sourceURL,
        snapshot,
        status: "submitted",
      })
      .returning();
    ...
    assert.deepEqual(
      (await getApplicationFile(owner, record.id, file.id))?.content,
      bytes,
    );
    const { GET } =
      await import("../app/api/applications/[id]/files/[fileId]/route");
    assert.equal(
      (
        await GET(
          new Request("http://localhost:3000/api/applications/test/files/test"),
          { params: Promise.resolve({ id: record.id, fileId: file.id }) },
        )
      ).status,
      401,
    );
    console.log(
      "PASS: saved disabled fields, option/free-text answers, attachment links, database snapshots, owner isolation, exact attachment bytes, and unauthenticated download rejection.",
    );
  } finally {
    await pool.end();
...
main().catch((error) => {
  console.error(
    error instanceof Error ? error.message : "Applications test failed",
  );
  process.exitCode = 1;
});
